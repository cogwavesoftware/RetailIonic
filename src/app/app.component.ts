...
export class MyApp {
  ...
  constructor(
    ...
    private events: Events,private _lap :InAppBrowser,
    private translateService: TranslateService
  ) {
    //this.initializeApp();
    localStorage.setItem('currency', JSON.stringify({ currencyName: 'INR', currencySymbol: 'Rs' }));
    localStorage.setItem("uid","Francis");
    platform.ready().then(() => {
      setTimeout(() => {
        statusbar.hide();
        splashscreen.hide();
  ...
  aboutUs() {
    this._lap.create("http://cogwave.in/",'_blank');
    //this.nav.push("AboutUsPage");
  }
  settings() {
  ...
  isLoggedin() {
    localStorage.setItem("uid","ABCDES");
    return localStorage.getItem("uid") != null;
  }

  IsMobilein() {
   
    return localStorage.getItem("MobileNo") != null;
  }
